...
public class Course {
    ...
    public Course(
            ...
            Long categoryId,
            String totalSeconds,
            int totalLectureCount) { // totalSeconds 추가
        this.id = id;
        this.title = title;
        this.instructorId = instructorId;
    ...
    public static Course create(
        ...
        CourseDetail courseDetail = new CourseDetail(content, contentDetail);

        return new Course(title, instructorId, courseDetail, categoryId, sections, tags);
    }
    ...
    public void addSection(Section section) {
        ...
        int nextOrder = sections.stream().mapToInt(Section::getSectionOrder).max().orElse(0) + 1;
    ...
    private void updateTotalDuration() {
        long totalSeconds = this.sections.stream().mapToInt(Section::calculateSectionDuration).sum();

        this.totalSeconds = TimeConverter.getFormattedDuration(totalSeconds);
    }

    private void updateTotalLectureCount() {
        this.totalLectureCount = this.sections.stream().mapToInt(Section::calculateLectureCount).sum();
    }
