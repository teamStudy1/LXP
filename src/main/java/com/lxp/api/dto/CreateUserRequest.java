...
public record CreateUserRequest(
    ...
    public void validate() {
        ...
        final String PASSWORD_REGEX = "^(?=.*[!@#$%^&*(),.?\":{}|<>\\[\\]\\/\\\\;'+\\-=~_`])(?=.*[A-Za-z0-9])[A-Za-z0-9!@#$%^&*(),.?\":{}|<>\\[\\]\\/\\\\;'+\\-=~_`]{8,}$";
        if (!Pattern.matches(PASSWORD_REGEX, password)) throw new IllegalStateException("비밀번호는 영어와 숫자만 가능하며 특수 문자 최소 1개를 포함하여 8자리 이상이 되어야 합니다.");
